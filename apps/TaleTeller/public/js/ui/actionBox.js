export function showActionBox(config) {
    ...
    if (document.getElementsByTagName('actionBoxOverlay')) return;

    const { title, message, actions, type = 'neutral', dismissible = true } = config;

    if (!title || !message || !Array.isArry(actions) || actions.length === 0) {
    ...
    actions.forEach(({ label, action }) => {
        ...
        actionsContainer.appenChild(btn);
    });

    box.appendChild(h3);
    box.appendChild(p);
    box.appendChild(actionsContainer);
    overlay.appenChild(box);
